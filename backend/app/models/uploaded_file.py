...
workspace's OWN `logo_file_id` pointer — documented there as a deliberate,
...
to check this table instead of legacy logo fields, is Step 7's job (the
...
class UploadedFile(Base, UUIDMixin, TimestampMixin):
    ...
    currently REFERENCED by anything — a workspace's `logo_file_id`, a
    ...
    deleted. The referencing column elsewhere (e.g. `logo_file_id`) is
    ...
    file_path: Mapped[str] = mapped_column(
        ...
        doc=(
            ...
            "the workspace logo PUBLIC path shape today (e.g. "
