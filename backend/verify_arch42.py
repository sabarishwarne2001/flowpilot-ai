...
def check_wiring(texts: dict[str, str]) -> None:
    ...
    assert 'REVIEW_KIND_MERGE: str = "MERGE"' in texts["review_model"] and (
        ...
        or "    REVIEW_KIND_MERGE,\n    REVIEW_KIND_SPLIT,\n    REVIEW_KIND_TABLE,\n    REVIEW_KIND_CORROBORATION,\n    REVIEW_KIND_OBLIGATION,\n)" in texts["review_model"])  # ARCH44-S1:review-kinds-widened-42  ARCH45-S1:review-kinds-widened-42  ARCH46-S1:review-kinds-widened-42
    ...
    newest46 = VERSIONS / "arch46_step1_obligations.py"  # ARCH46-S1:vocab-widened-42
    ref = _load_module("_m46", newest46) if newest46.exists() else ref
...
def db_layer(rec: Recorder, evidence: dict, mutate: bool) -> None:
    ...
    def head() -> None:
        ...
        assert current in ([A42], [STEP3], ["arch43_step1_case_intelligence"], ["arch44_step1_table_intelligence"], ["arch45_step1_corroboration"], ["arch46_step1_obligations"]), f"alembic current is {current}; run run_arch42.ps1"  # ARCH43-S1:head-widened-42  ARCH44-S1:head-widened-42  ARCH45-S1:head-widened-42  ARCH46-S1:head-widened-42
