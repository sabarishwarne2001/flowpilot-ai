...
# ===========================================================================
# A9 — seeded, rolled-back end-to-end database gates
# ===========================================================================
...
def gates_a9(rec: Recorder, database_url: str) -> None:
    ...
    class _StubbedDodo:
        ...
        somebody else's uptime. The stub returns a snapshot shaped exactly
        like `DodoSubscriptionSnapshot` so the reconcile path exercises real
    ...
    def dodo_inbound_reconciles() -> None:
        from app.models.billing import BillingInboundEvent  # noqa: F401
        ...
        with Session(engine) as db:
            ...
            try:
                ...
                row = _seed_inbound_dodo_event(db, org_id, "subscription.active", "sub_a9_active")

                dodo_reconcile_service.reconcile_row(
                    db, row=row, gateway=_StubbedDodo(snapshot)
                )
                db.flush()

                sub = _fetch_subscription(db, org_id)
                assert sub is not None, (
                    "reconcile_row did not create a subscription row for an "
                    "inbound subscription.active event"
                )
                ...
                assert sub.quota_tier_id is not None, (
                    "tier pin is NULL after reconcile; quota_service."
                    "resolve_tier then falls back and the customer silently "
                    "loses the limits they are paying for"
                )
            finally:
                outer.rollback()
    ...
    # -- A9.2 Stripe cancellation lapse --------------------------------
    def stripe_cancellation_lapses_to_free() -> None:
        ...
        with Session(engine) as db:
            ...
            try:
                org_id, tier_id = _seed_org_and_tier(db)
                sub = _seed_stripe_subscription(db, org_id, tier_id, status="active")
                _pin_tier(db, org_id, tier_id)

                subscription_service.apply_tier_for_status(
                    db, subscription=sub, status="canceled"
                )
                db.flush()

                pinned = _pinned_tier_key(db, org_id)
                assert pinned == "free", (
                    f"organization is still pinned to {pinned!r} after "
                    f"cancellation. This is the Tranche 3 defect: "
                    f"quota_service.resolve_tier falls back to the pointer "
    ...
    rec.check("A9.2 Stripe cancellation lapses the tier pin to free", stripe_cancellation_lapses_to_free)

    # -- A9.3 Add-on sweep lifecycle -----------------------------------
    def addon_sweep_revokes_and_disables() -> None:
        ...
        with Session(engine) as db:
            ...
            try:
                ...
                grant = _seed_addon_grant(db, org_id, "addon.custom_domain", state="ACTIVE")
                domain_id = _seed_verified_domain(db, org_id)
                schedule_id = _seed_export_schedule(db, org_id)

                # ACTIVE -> GRACE
                _age_grant(db, grant, days=1)
                addon_service.sweep(db)
                db.flush()
                assert _grant_state(db, grant) == "GRACE", _grant_state(db, grant)

                # GRACE -> LAPSED, and the halt effects must actually fire
                _age_grant(db, grant, days=99)
                addon_service.sweep(db)
                db.flush()
                assert _grant_state(db, grant) == "LAPSED", _grant_state(db, grant)
                ...
                assert not _schedule_is_enabled(db, schedule_id), (
                    "the export schedule is still enabled after the add-on "
                    "lapsed; it will keep shipping data the tenant no longer "
                    "pays for"
                )
            finally:
                outer.rollback()

    rec.check("A9.3 add-on sweep walks ACTIVE -> GRACE -> LAPSED and fires halt effects", addon_sweep_revokes_and_disables)


# ---------------------------------------------------------------------------
# A9 seed helpers
#
# Every one of these writes inside the caller's transaction and never commits.
# The gate's outer `begin()` is rolled back unconditionally in `finally`, so a
# gate that fails mid-way leaves the database exactly as it found it — which
# is what makes it safe to point --db at a development database with real
# rows in it.
# ---------------------------------------------------------------------------


def _seed_org_and_tier(db: Any) -> tuple[uuid.UUID, uuid.UUID]:
    from sqlalchemy import text as sa_text

    org_id = db.execute(
        sa_text("SELECT id FROM organizations ORDER BY created_at LIMIT 1")
    ).scalar_one_or_none()
    tier_id = db.execute(
        sa_text(
            "SELECT id FROM quota_tiers WHERE is_published = true "
            "ORDER BY created_at DESC LIMIT 1"
        )
    ).scalar_one_or_none()
    if org_id is None or tier_id is None:
        raise AssertionError(
            "A9 needs at least one organization and one published quota tier "
            "in the target database. Run the development seed first; these "
            "gates deliberately do not invent tenancy rows, because a gate "
            "that creates its own organization is not exercising the same "
            "constraints production rows live under."
        )
    return org_id, tier_id


def _seed_inbound_dodo_event(db: Any, org_id: uuid.UUID, event_type: str, sub_id: str) -> Any:
    raise AssertionError(
        "_seed_inbound_dodo_event is not implemented in this delivery. See "
        "the A9 note in the accompanying summary: the inbound-event row shape "
        "must be read off app/models/billing.py against a live schema before "
        "it can be seeded honestly, and that was not done in this session."
    )


def _fetch_subscription(db: Any, org_id: uuid.UUID) -> Any:
    from sqlalchemy import text as sa_text

    return db.execute(
        sa_text(
            "SELECT * FROM subscriptions WHERE organization_id = :org "
            "ORDER BY created_at DESC LIMIT 1"
        ),
        {"org": str(org_id)},
    ).mappings().first()
...
def _seed_stripe_subscription(db: Any, org_id: uuid.UUID, tier_id: uuid.UUID, *, status: str) -> Any:
    raise AssertionError("_seed_stripe_subscription is not implemented in this delivery.")


def _pin_tier(db: Any, org_id: uuid.UUID, tier_id: uuid.UUID) -> None:
    from sqlalchemy import text as sa_text

    db.execute(
        sa_text("UPDATE organizations SET quota_tier_id = :tier WHERE id = :org"),
        {"tier": str(tier_id), "org": str(org_id)},
    )


def _pinned_tier_key(db: Any, org_id: uuid.UUID) -> Optional[str]:
    from sqlalchemy import text as sa_text
...
def _seed_addon_grant(db: Any, org_id: uuid.UUID, key: str, *, state: str) -> Any:
    raise AssertionError("_seed_addon_grant is not implemented in this delivery.")


def _seed_verified_domain(db: Any, org_id: uuid.UUID) -> uuid.UUID:
    raise AssertionError("_seed_verified_domain is not implemented in this delivery.")


def _seed_export_schedule(db: Any, org_id: uuid.UUID) -> uuid.UUID:
    raise AssertionError("_seed_export_schedule is not implemented in this delivery.")


def _age_grant(db: Any, grant: Any, *, days: int) -> None:
    raise AssertionError("_age_grant is not implemented in this delivery.")


def _grant_state(db: Any, grant: Any) -> str:
    raise AssertionError("_grant_state is not implemented in this delivery.")


def _domain_is_revoked(db: Any, domain_id: uuid.UUID) -> bool:
    raise AssertionError("_domain_is_revoked is not implemented in this delivery.")


def _schedule_is_enabled(db: Any, schedule_id: uuid.UUID) -> bool:
    raise AssertionError("_schedule_is_enabled is not implemented in this delivery.")
