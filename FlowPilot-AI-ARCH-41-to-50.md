...
<!-- ARCH46-S1:roadmap-status -->
**Status:** ARCH-41 through ARCH-46 delivered and certified (see `ARCH-41-FINAL-CERTIFICATION.md` through `ARCH-46-FINAL-CERTIFICATION.md`). ARCH-47 to ARCH-50 specified here.
**Alembic after ARCH-46:** `hm1_tier_price_per_key → arch41_step1_extraction_memory → arch42_step1_entity_graph → arch43_step1_case_intelligence → arch44_step1_table_intelligence → arch45_step1_corroboration → arch46_step1_obligations → arch40_step3_contract_ai_settings` (held). One file head; the release head is `arch46_step1_obligations`.
...
| 46 | Obligations & Temporal Intelligence — DONE | `capability.obligations` | Business+ | 42 |
...
### ARCH-46 — Obligations & Temporal Intelligence (`capability.obligations`, Business+) — delivered
...
**As built.** One migration, `arch46_step1_obligations` (the contract step re-parented onto it), with the four tables as specified plus a `work_items` trigger that unlinks a person's own obligations before a document is deleted (what was READ from a document cascades with it). Extraction is deterministic and model-free: ARCH-45's clause segmentation and typed values, ARCH-33's compiled notice-period reader as a cross-check, and date arithmetic whose seven rules are stated in `obligations/temporal.py` (month ends clamp, series are anchored not chained, business days never count the start day, notices roll PRECEDING, a due date is a LOCAL date in the workspace's zone). Holiday calendars are stored rows filled from built-in templates (US federal with OPM observance, England & Wales with substitute days, India national, weekends only), pasted dates or an .ics file — nothing is fetched. Obligations read with a doubt (ambiguous DD/MM vs MM/DD, business days without a holiday calendar, an ARCH-33 disagreement) are an eighth review-hub kind (OBLIGATION, reason OBLIGATION_UNCONFIRMED) and reach Flow Builder only once confirmed. The sweep runs HOURLY in UTC and evaluates each workspace at its own local date; each alert is exactly once per obligation, state and due date (a partial UNIQUE index plus the outbox idempotency key), so a moved due date alerts again and a re-run never does. Two Flow Builder triggers (`obligation.due_soon`, `obligation.overdue`; 21 triggers over 22 events; live conformance ALL CONFORMANT). Feeds: `v1.<random>.<HMAC under HKDF of the configured keys>`, only the SHA-256 stored, every refusal the identical uncached 404, revocation effective on the next fetch; tokens in public paths are now redacted from the app's logs, uvicorn's access log and the Caddy access log. Recall and precision 100% on 10 golden documents (22 obligations) and 300 held-out documents (seeds 4700–4999) — synthetic documents only; real-world accuracy is unmeasured.
...
ARCH-42 was started from `ARCH-42-HANDOFF-PROMPT.md`, ARCH-43 from `ARCH-43-HANDOFF-PROMPT.md`, ARCH-44 from `ARCH-44-HANDOFF-PROMPT.md` and ARCH-45 from `ARCH-45-HANDOFF-PROMPT.md`. ARCH-46 was started from `ARCH-46-HANDOFF-PROMPT.md`; the self-contained prompt for ARCH-47 is in `ARCH-47-HANDOFF-PROMPT.md`.
