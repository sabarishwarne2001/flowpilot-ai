...
def check_migration(text: str, revs: Optional[dict] = None) -> None:
    ...
    assert revs.get(STEP3) == f'"{A42}"', f"the contract step revises {revs.get(STEP3)}, expected {A42}"
...
def check_wiring(texts: dict[str, str]) -> None:
    ...
    assert 'REVIEW_KIND_MERGE: str = "MERGE"' in texts["review_model"] and "    REVIEW_KIND_MERGE,\n)" in texts["review_model"]
    ...
    assert tuple(REVIEW_KINDS) == m42.REVIEW_KINDS and tuple(vocab.REASONS) == m42.REVIEW_REASONS, "hub vocabulary != migration"
...
def db_layer(rec: Recorder, evidence: dict, mutate: bool) -> None:
    ...
    def head() -> None:
        ...
        assert current in ([A42], [STEP3]), f"alembic current is {current}; run run_arch42.ps1"
