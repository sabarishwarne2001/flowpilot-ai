...
EMITTERS: dict[str, tuple[str, tuple[str, ...]]] = {
    ...
    # ARCH44-S1:emitters. A table entering FLAGGED.
    "trigger.table.flagged": (
        "app/services/tables/service.py",
        ("emit_trigger(", "event_type=v.EVENT_TABLE_FLAGGED"),
    ),
    # ARCH45-S1:emitters. A completed comparison with material differences.
    "trigger.corroboration.discrepancies": (
        "app/services/corroboration/service.py",
        ("emit_trigger(", "event_type=v.EVENT_DISCREPANCIES"),
    ),
...
def gates_offline(rec: Recorder, *, root: Path, only: set[str] | None) -> None:
    ...
    if want("V1"):
        def vocabulary() -> None:
            ...
            assert (len(triggers.TRIGGERS), len(triggers.CATALOG_EVENT_TYPES)) in ((13, 14), (14, 15), (17, 18), (18, 19), (19, 20))  # ARCH43-S1:catalog-counts-37  ARCH44-S1:catalog-counts-37  ARCH45-S1:catalog-counts-37
...
def gates_db(rec: Recorder, *, root: Path) -> None:
    ...
    try:
        def head() -> None:
            ...
            assert value in (HEAD, "arch38_step1_batches", "arch40_step2_settings_backfill", "arch40_step2a_review_view_paths", "arch40_step3_contract_ai_settings", "hm1_tier_price_per_key", "arch41_step1_extraction_memory", "arch42_step1_entity_graph", "arch43_step1_case_intelligence", "arch44_step1_table_intelligence", "arch45_step1_corroboration"), f"alembic head is {value}; run `alembic upgrade head`"  # ARCH40-S1:head-widened-37  # HM-S1:head-widened  ARCH41-S2:head-widened-37  ARCH42-S1:head-widened-37  ARCH43-S1:head-widened-37  ARCH44-S1:head-widened-37  ARCH45-S1:head-widened-37
