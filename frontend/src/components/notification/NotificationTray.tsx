...
export const NotificationTray: React.FC<NotificationTrayProps> = ({
  ...
  const {
    ...
    enabled: isOpen, // Only query when the user opens the popover drawer
    staleTime: 1000 * 10, // 10-second data freshness boundaries
    placeholderData: (previousData) => previousData, // Suppresses visual flickers on refetch
  });
  ...
  return (
    ...
    >
      ...
      <header className="p-4 border-b border-border/40 flex items-center justify-between bg-muted/5 select-none">
        ...
        <div className="flex items-center space-x-2">
          {/* Mark all read trigger (visible only if there are unread items) */}
          ...
          {/* Localized popover close button */}
          <button
            type="button"
            onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-border/20 scrollbar bg-background/50">
        {isLoading ? (
          ...
          /* Connectivity alert error panel */
          ...
          /* Dormant empty tray notifications panel */
          <div className="p-8 text-center select-none text-muted-foreground">
          ...
          notifications.map((alert) => (
            ...
            >
              {/* Event category card icon wrapper */}
              ...
              {/* Individual alert dismiss delete icon */}
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation(); // Prevent redirect trigger actions
