"""ARCH46-S1:gate — may obligations run for this organization?"""
...
logger = logging.getLogger("app.services.obligations.gate")
...
def capability_held(db: Session, organization_id: uuid.UUID) -> bool:
    ...
    try:
        return capability_gate.has_capability(
            db, organization_id=organization_id, capability_key=entitlements.OBLIGATIONS_CAPABILITY)
    except Exception:  # noqa: BLE001 - an unreadable plan is "not held", never a crash in the pipeline
        logger.exception("obligations.capability_unreadable")
        return False
