<#
.SYNOPSIS
  FlowPilot AI - Hardening Tier 3: check, apply, seed, build and verify.

.DESCRIPTION
  Place this file, apply_hardening_tier3.py and the certification document in
  ...
  -> npm ci -> verify_hardening_tier3.py (--db --mutation --frontend [--chain]).

  The --db gates create organizations named "hardening-t3-gate ..." in the
...
& $Python (Join-Path $Root "apply_hardening_tier3.py") --root $Root --check
if ($LASTEXITCODE -ne 0) { Fail "the tree is not at the tier-2 state (apply tiers 1 and 2 first). Nothing was written." }
if ($CheckOnly) { Write-Host "Check only: no files were written." -ForegroundColor Green; exit 0 }

Step "2/8 Apply"
& $Python (Join-Path $Root "apply_hardening_tier3.py") --root $Root
...
$second = & $Python (Join-Path $Root "apply_hardening_tier3.py") --root $Root 2>&1 | Out-String
...
try {
    ...
    Write-Host "Tier 3 adds no plan changes."
}
...
Push-Location $Backend
try {
    $verifyArgs = @("verify_hardening_tier3.py", "--mutation", "--previous")
...
Step "8/8 Summary"
if ($verifyExit -eq 0) {
    Write-Host "HARDENING TIER 3: ALL GATES PASSED" -ForegroundColor Green
    Write-Host "Rollback, if ever needed: $Python apply_hardening_tier3.py --rollback"
    exit 0
}
Write-Host "HARDENING TIER 3: GATES FAILED (see output above)." -ForegroundColor Red
Write-Host "Rollback: $Python apply_hardening_tier3.py --rollback"
exit 1
