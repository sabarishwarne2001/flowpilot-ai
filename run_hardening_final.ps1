<#
.SYNOPSIS
  FlowPilot AI - Hardening Final: check, apply, seed, build and verify.

.DESCRIPTION
  Place this file, apply_hardening_final.py and the certification document in
  ...
  -> npm install -> verify_hardening_final.py (--db --mutation --frontend [--chain]).

  The --db gates create organizations named "hardening-t3-gate and billing-gate ..." in the
...
& $Python (Join-Path $Root "apply_hardening_final.py") --root $Root --check
if ($LASTEXITCODE -ne 0) { Fail "the tree is not at the tier-3 state (apply tiers 1-3 first). Nothing was written." }
if ($CheckOnly) { Write-Host "Check only: no files were written." -ForegroundColor Green; exit 0 }

Step "2/8 Apply"
& $Python (Join-Path $Root "apply_hardening_final.py") --root $Root
...
$second = & $Python (Join-Path $Root "apply_hardening_final.py") --root $Root 2>&1 | Out-String
...
try {
    ...
    Write-Host "The final package adds no plan changes."
}
finally { Pop-Location }

Step "6/8 Frontend dependencies (npm install: avoids the Windows esbuild.exe lock that npm install hits)"
if (-not $SkipFrontend) {
...
Push-Location $Backend
try {
    $verifyArgs = @("verify_hardening_final.py", "--mutation", "--previous")
...
Step "8/8 Summary"
if ($verifyExit -eq 0) {
    Write-Host "HARDENING FINAL: ALL GATES PASSED" -ForegroundColor Green
    Write-Host "Rollback, if ever needed: $Python apply_hardening_final.py --rollback"
    exit 0
}
Write-Host "HARDENING FINAL: GATES FAILED (see output above)." -ForegroundColor Red
Write-Host "Rollback: $Python apply_hardening_final.py --rollback"
exit 1
