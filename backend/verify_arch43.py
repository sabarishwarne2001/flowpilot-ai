...
def check_migration(text: str, revs: Optional[dict] = None) -> None:
    ...
    assert revs.get(STEP3) == f'"{A43}"', f"the contract step revises {revs.get(STEP3)}, expected {A43}"
    ...
    assert set(module.internal_after_43()) == set(ae.TRIGGER_NATIVE_EVENT_TYPES) | set(ae.TRIGGER_TWIN_EVENT_TYPES) | \
        (set(module.internal_after_43()) - set(ae.TRIGGER_NATIVE_EVENT_TYPES) - set(ae.TRIGGER_TWIN_EVENT_TYPES))
...
def check_triggers(texts: dict[str, str]) -> None:
    ...
    assert (len(triggers.TRIGGERS), len(triggers.CATALOG_EVENT_TYPES)) == (17, 18), "catalog is not 17 triggers over 18 events"
    ...
    assert "EXPECTED_TRIGGERS = 17" in texts["conformance"], "the live conformance matrix still expects 14"
    assert '"trigger.case.completed": (' in texts["verify37"] and "(17, 18)" in texts["verify37"]
    assert "assert len(specs) in (14, 17)" in texts["verify41"]
...
def db_layer(rec: Recorder, evidence: dict, mutate: bool) -> None:
    ...
    def head() -> None:
        ...
        assert current in ([A43], [STEP3]), f"alembic current is {current}; run run_arch43.ps1"
...
def mutations() -> list[tuple[str, Callable[[], None]]]:
    ...
    return [
        ...
        ("M16 the conformance matrix still expects 14 triggers", mutation(lambda: (texts_with("conformance", swap(
            t("conformance"), "EXPECTED_TRIGGERS = 17", "EXPECTED_TRIGGERS = 14")),), check_triggers)),
