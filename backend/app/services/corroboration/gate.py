"""ARCH45-S1:gate — may the corroborator run for this organization?"""
...
logger = logging.getLogger("app.services.corroboration.gate")
...
def capability_held(db: Session, organization_id: uuid.UUID) -> bool:
    ...
    try:
        return capability_gate.has_capability(
            db, organization_id=organization_id, capability_key=entitlements.UNIVERSAL_CORROBORATOR_CAPABILITY)
    except Exception:  # noqa: BLE001 - an unreadable plan is "not held", never a crash in the pipeline
        logger.exception("corroboration.capability_unreadable")
        return False
