...
def test_review_queue_lists_disagreed(
    ...
    work_item = work_item_factory()
    _disagreed(db_session, tenant, work_item)

    response = client.get(
        f"/api/v1/workspaces/{tenant.workspace.id}/verifications?status=DISAGREED",
        headers=_headers(tenant.contributor),
    )
...
def test_cross_tenant_read_returns_404(
    ...
    response = client.get(
        f"/api/v1/workspaces/{tenant.foreign_workspace.id}/verifications/{verification.id}",
        headers=_headers(tenant.other_org_member),
    )
    assert response.status_code == 404


def test_viewer_cannot_resolve(client, db_session, tenant, work_item_factory) -> None:
    work_item = work_item_factory()
    verification = _disagreed(db_session, tenant, work_item)

    response = client.post(
        f"/api/v1/workspaces/{tenant.workspace.id}/verifications/{verification.id}/resolve",
        headers=_headers(tenant.viewer),
...
def test_resolve_endpoint_releases_the_work_item(
    ...
    assert dv.blocking_verification(db_session, work_item_id=work_item.id) is None
...
def test_empty_resolve_is_rejected(
    client, db_session, tenant, work_item_factory
) -> None:
    work_item = work_item_factory()
    verification = _disagreed(db_session, tenant, work_item)
