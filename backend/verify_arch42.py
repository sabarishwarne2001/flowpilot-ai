...
def check_migration(text: str, revs: Optional[dict] = None) -> None:
    ...
    assert revs.get(STEP3) in (f'"{A42}"', '"arch43_step1_case_intelligence"'), f"the contract step revises {revs.get(STEP3)}, expected {A42} or arch43"
...
def check_wiring(texts: dict[str, str]) -> None:
    ...
    ref = _load_module("_m43", newest) if newest.exists() else m42
...
def db_layer(rec: Recorder, evidence: dict, mutate: bool) -> None:
    ...
    def head() -> None:
        ...
        assert current in ([A42], [STEP3], ["arch43_step1_case_intelligence"]), f"alembic current is {current}; run run_arch42.ps1"  # ARCH43-S1:head-widened-42
