<#
.SYNOPSIS
  FlowPilot AI - Hardening Tier 1: check, apply, seed, build and verify.

.DESCRIPTION
  Place this file, apply_hardening_tier1.py and the certification document in
  ...
  -> seed quota tiers (new version carrying capability.* entries, D29)
  -> npm ci -> verify_hardening_tier1.py (--db --mutation --frontend [--chain]).

  The --db gates create organizations named "hardening-t1-gate ..." in the
...
& $Python (Join-Path $Root "apply_hardening_tier1.py") --root $Root --check
if ($LASTEXITCODE -ne 0) { Fail "the tree is not at 960dd4d (or already hardened). Nothing was written." }
if ($CheckOnly) { Write-Host "Check only: no files were written." -ForegroundColor Green; exit 0 }

Step "2/8 Apply"
& $Python (Join-Path $Root "apply_hardening_tier1.py") --root $Root
...
$second = & $Python (Join-Path $Root "apply_hardening_tier1.py") --root $Root 2>&1 | Out-String
...
try {
    ...
    Step "5/8 Quota tiers carrying capabilities (D29)"
    if (-not $NoDb) {
        $seedArgs = @("scripts\seed_quota_tiers.py", "--version", "$TierVersion")
        if ($AllowUnpriced) { $seedArgs += "--allow-unpriced" }
        & $Python @seedArgs
        if ($LASTEXITCODE -ne 0) {
            Write-Host "WARNING: seeding tier version $TierVersion failed. If that version already exists, re-run with -TierVersion <next>." -ForegroundColor Yellow
        }
        Write-Host "Existing organizations stay pinned to their current tier version; see the certification document, section 'Plans'." -ForegroundColor Yellow
    } else { Write-Host "skipped (-NoDb)" }
}
...
Push-Location $Backend
try {
    $verifyArgs = @("verify_hardening_tier1.py", "--mutation")
...
Step "8/8 Summary"
if ($verifyExit -eq 0) {
    Write-Host "HARDENING TIER 1: ALL GATES PASSED" -ForegroundColor Green
    Write-Host "Rollback, if ever needed: $Python apply_hardening_tier1.py --rollback"
    exit 0
}
Write-Host "HARDENING TIER 1: GATES FAILED (see output above)." -ForegroundColor Red
Write-Host "Rollback: $Python apply_hardening_tier1.py --rollback"
exit 1
